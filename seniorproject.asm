;********************************************************************
; Filename: SeniorProject											*
; Function: Gesture Controlled Fan								*
; Author: Devion WIlson												*
; Date: 09-06-2016													*
;********************************************************************
		#include <p16f877a.inc>	;This is the header file you created for this lab#1



FANSET	EQU		0X20
OSCC	EQU		0X21
HIGHBT	EQU		0X24
LOWBT	EQU		0X25
TESTBIT	EQU		0X26
KEY		EQU		0X2C		;This is the user defined DRAM area that
COUNT	EQU		0X2D		; uses the 68 bytes GPR: range from 0X0C-0X4F
TEMP	EQU		0X2E
TEMP1	EQU		0X2F
TEMP2	EQU		0X30
DVAR	EQU		0X31
DVAR2	EQU		0X32
FLAG	EQU		0X33
;TIME 	EQU		0X9F
VAR1	EQU		0X34
VAR2	EQU		0X35
VAR3 	EQU		0X36
STEMP	EQU		0X37
WTEMP	EQU		0X38
ORIG	EQU		0X39
NEW		EQU		0X3A
;CHANGE	EQU		0X1B
N		EQU		0X3C
CONT	EQU		0X3D
CONT1	EQU		0X3E
TESTBITS	EQU		0X3F
RETULT	EQU		0X40

BYTE	EQU		0X42
count	EQU		0X43

	    
	    	__CONFIG	0X3F32
	    	ORG		0X0000					;RESET or WDT reset vector

	

			
GOTO	START
		    ORG		0X0004					;Regular INT vector
			
		

			CALL	DELAY1
									;This is the area where your interrupt service routine goes
			MOVWF	WTEMP				;SAVE W AND status
			SWAPF	STATUS,W
			MOVWF	STEMP
			
			BTFSC	INTCON,RBIF
			INCF	FLAG,F
			BTFSC	INTCON,INTF
			BSF	DVAR2,0X0			;SET FLAG TO DISPLAY LCD MESSAGE
			
			BTFSC	INTCON,INTF
			INCF	CONT1,F				;INCREMENT COUNTER IF INTF IS SET
			
			SWAPF	STEMP,W				;POP STACK
			MOVWF	STATUS
			SWAPF	WTEMP,F
			SWAPF	WTEMP,W
			MOVF	PORTB,W				;DUMMY OPERATION
			BCF		INTCON,INTF			;CLEAR INTURRUPT FLAG
			BCF		INTCON,RBIF
			RETFIE	


START	
			
			BSF 	STATUS,RP0 			;GO TO BANK 1
			
			MOVLW	0X00
			MOVWF   TRISA				;SET 0-3 FOR INPUT AND 4-7 FOR OUTPUT
			MOVLW	0X81
			MOVWF	TRISB				;SET 0-3 FOR INPUT AND 4-7 FOR OUTPUT
			MOVLW	0X00
			MOVWF	TRISC
			MOVLW	0X00
			MOVWF	TRISD
			MOVLW	0X00					;RE2 as input pin for ADC COnversion
			MOVWF	TRISE
		
	    		BCF		STATUS,RP0	
			BCF		PORTE,0X0
			BCF		PORTE,0X1
			BSF		INTCON,GIE
			BSF		INTCON,INTE
			BSF		INTCON,RBIE
			BSF		OPTION_REG,INTEDG
			MOVLW    0XFA
			MOVWF	N					;OSCILATION COUNTER
			MOVLW	0X00
			MOVWF	CONT1
			MOVWF	PORTD
			MOVLW	0X01
			MOVWF	RETULT				;SET THE MOTOR DIRECTION (RESULT) 00000001
			MOVWF	DVAR
			MOVLW	0X03
			MOVWF	TESTBITS				;SET THE TESTBIT 00000011
			
STARTER		

			BTFSC	DVAR2,0X0			;CHECK IF SETTING WAS RECENTLY CHANGED
			GOTO	LCDDIS				;IFSO,DISPLAY MESSAGE ON THE LCD!!
							
			MOVLW	0X00
			XORWF	CONT1,0				;CHECK IF BIT IS SET
			BTFSC	STATUS,Z
			call	MOTOR0
			MOVLW	0X01
			XORWF	CONT1,0				;CHECK IF BIT IS SET
			BTFSC	STATUS,Z
			call	MOTOR1
			MOVLW	0X02
			XORWF	CONT1,0				;CHECK IF BIT IS SET
			BTFSC	STATUS,Z
			call	MOTOR2
			MOVLW	0X03
			XORWF	CONT1,0				;CHECK IF BIT IS SET
			BTFSC	STATUS,Z
			call	MOTOR3	
	
			
			MOVLW	0X04
			SUBWF	CONT1,0				;CHECK IF COUNTER HAS PASSED 8
			BTFSC	STATUS,C
			CALL	FIX

			MOVF	FLAG,W				;Display the counter onto the LED 
			MOVWF		PORTA
			
			MOVLW	0X04
			SUBWF	FLAG,0
			BTFSC	STATUS,C
			CALL	FIX1				   ;CHECK IF COUNTR HAS PASSED 3, IF SO RESET THE COUNTER
			
								    ;CHECK IF COUNTRER IS ON 1, IF SO THEN SEND IT BACK TO 0
			;BTFSC		FLAG,0X0		    
			;CALL		FIX1
			
			BTFSC		FLAG,0X1				;OSCILATES THE MOTOR IF THE FLAG IS SET(VALUE MUST EQUAL 0X2)
			call		OSCL
			BTFSS		FLAG,0X1				;STOPS THE OSCILATION IF THE FLAG IS NOT SET
			CALL		STOPPER
			
			
          		GOTO		STARTER
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

STOPPER
			MOVLW		0X00
			MOVWF		PORTE
			RETURN
OSCL		;MOTOR OSCILATION FUNCTION
			
			BTFSC		DVAR,0X0
			CALL		OSCLREAL
			BTFSS		DVAR,0X0
			CALL		BLANKER
			
			RETURN
			
BLANKER
			
			DECFSZ		N					;SUPPOSED TO COUNT 150 TIMES,WHEN IT HITS 0 IT WILL BLANK BEFORE GOING IN THE OPPOSITE DIRECTION
			RETURN
			
			MOVLW		0X00
			MOVWF		PORTE	
		
			MOVLW		0XFF
			MOVWF		N					;RESET THE OSCILATIN MOTOR		
			MOVF		TESTBITS,W
			XORWF		DVAR,F
			

RETURN
OSCLREAL
			DECFSZ		N					;SUPPOSED TO COUNT 150 TIMES,WHEN IT HITS 0 IT WILL OSCILATE IN THE NEXT DIRECTION
			RETURN
			
				
			
			MOVLW		0XFF
			MOVWF		N					;RESET THE OSCILATIN MOTOR		
			MOVF		TESTBITS,W
			XORWF		RETULT,F
			XORWF		DVAR,F
			MOVF		RETULT,W				;LOAD THE RESULT ONTO PORT E WHICH SHOULD OSCILLATE THE MOTOR BACK AND FOURTH
			
			MOVWF		PORTE

RETURN
	




MOTOR0		
	
	         		BCF		PORTC,0X5
			BCF		PORTC,0X4
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			call DELAY1
			return
MOTOR1		
			BCF		PORTC,0X5           ;SET  PIN3 ON PORTC TO CLEAR
			BSF		PORTC,0X4           ;SET PIN 2 ON PORTC TO 1
		
			call	DELAY1
			call	DELAY1
			call	DELAY1
			call	DELAY1
			call	DELAY1
			
			BCF	PORTC,0X4
			call	DELAY1
			call	DELAY1
			call	DELAY1
			call	DELAY1
			call	DELAY1
return

            
MOTOR2		
			BCF		PORTC,0X5
			BSF		PORTC,0X4
	
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
	BCF		PORTC,0X4

call DELAY1
call	DELAY1
call	DELAY1

return
            

MOTOR3		
			BCF		PORTC,0X5
			BSF		PORTC,0X4

call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
call	DELAY1
	BCF		PORTC,0X4
call	DELAY1
return

FIX
			MOVLW	0X00				;RESET COUNTER
			MOVWF	CONT1
		
			RETURN           ;GOTO BEGINNING OF CYCLE

FIX1			MOVLW	0X00
			MOVWF	FLAG
			RETURN
			
DELAY1										;TIME DELAY
		
			MOVLW	0X0F
			MOVWF	VAR2
LOOP2			MOVLW	0X28

			MOVWF	VAR3
LOOP3		
			DECF	VAR3,F
			btfss	STATUS,Z
			GOTO	LOOP3
			DECF	VAR2,F
			BTFSS	STATUS,Z
			GOTO	LOOP2
return
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

LCDDIS
	

			BCF		PORTB,0X5
			CALL	BIT0
			CALL	ELINE
			CALL	DELAY10
			CALL	BIT0
			CALL	ELINE
			CALL	DELAY10	
			CALL	BIT0
			CALL	ELINE
			CALL	DELAY10		
			CALL	BIT1
			CALL	ELINE
			CALL	DELAY10			
			CALL	BIT2
			CALL	ELINE
			CALL	DELAY10		
			CALL	BIT3
			CALL	ELINE
			CALL	DELAY10		
			CALL	BIT4
			CALL	ELINE
			CALL	DELAY10		
			CALL	BIT5
			CALL	ELINE
			CALL	DELAY10	
			MOVLW	0X10
			MOVWF	PORTD
			CALL	ELINE
			CALL	DELAY10
			BSF		PORTB,0X5
			
			CLRF	NEW
			BCF	    DVAR2,0X0
			clrw
UDISPLAY
		    
		    CLRF    CONT   				 
 		    MOVF    NEW,W 						;Put message counter in W  
		    INCF    NEW,F  	        				 	;Increment message counter 
			
		
							;CHECK IF BIT IS SET
			BTFSS	CONT1,0X0
			CALL    NSHORT_TABLE 			;DISPLAY MESSAAGE FOR FIRST TESTBIT ( HIGH VOLTAGE)

			;CHECK IF BIT IS SET
			BTFSC	CONT1,0X0
			CALL	OSHORT_TABLE    		;DISPLAY THE MESSAGE FOR SECOND TESTBIT(MEDIUM VOLTAGE)

							;CHECK IF BIT IS SET
			BTFSS	CONT1,0X0
			CALL	TSHORT_TABLE 			;DISPLAY THE MESSAGE FOR THE THIRD TESTBIT(LOW VOLTAGE)
			
						;CHECK IF BIT IS SET
			BTFSC	CONT1,0X0
			CALL	THSHORT_TABLE			;DISPLAY THE MESSAGE FOR THE FOURTH TESTBIT(UNKNOWN VOLTAGE)
		   
			MOVWF   PORTD
			 
		    ADDLW   0X00 
		    BTFSC   STATUS,Z       			 	;Check to see if last byte was 0x00 
		    GOTO 	STARTER
  		    		
			CALL    ELINE              				 ;DISPLAY TO LCD
			CALL	DELAY10
			GOTO    UDISPLAY
NSHORT_TABLE
			
			BTFSC	CONT1,0X1
			RETURN
			
			
			MOVWF	CONT				;SAVE OFFSET
			
			MOVLW	HIGH HMSG			;GET HIGH BYTE
			MOVWF	PCLATH
			MOVF	CONT,W
			ADDLW	LOW HMSG			;;UPDATE PCL REG
			BTFSC	STATUS,C
			INCF	PCLATH,F	
			MOVWF	PCL
			
HMSG
			DT	" Running setting 1 with a duty cycle of 0  percent"
			DT	0X00
			RETLW	0X00

OSHORT_TABLE
			
			BTFSC	CONT1,0X1
			RETURN
			
			MOVWF	CONT				;SAVE OFFSET
			MOVLW	HIGH MMSG			;GET HIGH BYTE
			MOVWF	PCLATH
			MOVF	CONT,W
			ADDLW	LOW MMSG			;;UPDATE PCL REG
			BTFSC	STATUS,C
			INCF	PCLATH,F	
			MOVWF	PCL
			
MMSG
			DT	" Running setting 2 with a duty cycle of  30 percent"
			DT	0X00
			RETLW	0X00
TSHORT_TABLE
			
			BTFSS	CONT1,0X1
			RETURN

			MOVWF	CONT				;SAVE OFFSET
			MOVLW	HIGH LMSG			;GET HIGH BYTE
			MOVWF	PCLATH
			MOVF	CONT,W
			ADDLW	LOW LMSG			;;UPDATE PCL REG
			BTFSC	STATUS,C
			INCF	PCLATH,F	
			MOVWF	PCL
			
LMSG
			DT	" Running setting 3 with a duty cycle of  70 percent"
			DT	0X00
			RETLW	0X00
THSHORT_TABLE
			
			BTFSS	CONT1,0X1
			RETURN
			
			MOVWF	CONT				;SAVE OFFSET
			MOVLW	HIGH MSG				;GET HIGH BYTE
			MOVWF	PCLATH
			MOVF	CONT,W
			ADDLW	LOW MSG				;;UPDATE PCL REG
			BTFSC	STATUS,C
			INCF	PCLATH,F	
			MOVWF	PCL
			
MSG
			DT	" Running setting 4 with a duty cycle of  90 percent"
			DT	0X00
			RETLW	0X00

ELINE
			BSF		PORTB,0X4			;TOGGLE E LINE
			NOP
			NOP
			NOP
			BCF		PORTB,0X4	
			NOP
			NOP
			NOP
			BSF		PORTB,0X4
			RETURN

BIT0										;SEND INITIALZATION BITS
			MOVLW	0X30
			MOVWF	PORTD
			RETURN
			
BIT1
			MOVLW	0X38
			MOVWF	PORTD
			RETURN
			
BIT2			MOVLW	0X0C
			MOVWF	PORTD
			RETURN
BIT3
			MOVLW	0X01
			MOVWF	PORTD
			RETURN
			
BIT4			MOVLW	0X06
			MOVWF	PORTD
			RETURN
			
BIT5	
			MOVLW	0X02
			MOVWF	PORTD
			RETURN

DELAY10									;TIME DELAY
		
			MOVLW	0X61
			MOVWF	VAR2
LOOP23			MOVLW	0X00

			MOVWF	VAR3
LOOP33		
			DECF	VAR3,F
			btfss	STATUS,Z
			GOTO	LOOP33
			DECF	VAR2,F
			BTFSS	STATUS,Z
			GOTO	LOOP2
            RETURN
	END